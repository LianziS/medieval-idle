...
const EXP_TABLE = {
    ...
    91: 4566274, 92: 4987741, 93: 5446463, 94: 5945587, 95: 6488521, 96: 7078945, 97: 7720834, 98: 8418485, 99: 9176537, 100: 10000000,
    101: 11404976, 102: 12904567, 103: 14514400, 104: 16242080, 105: 18095702, 106: 20083886, 107: 22215808, 108: 24501230, 109: 26950540, 110: 29574787,
    111: 32385721, 112: 35395838, 113: 38618420, 114: 42067584, 115: 45758332, 116: 49706603, 117: 53929328, 118: 58444489, 119: 63271179, 120: 68429670,
    121: 73941479, 122: 79829440, 123: 86117783, 124: 92832214, 125: 100000000,
    126: 114406130, 127: 130118394, 128: 147319656, 129: 166147618, 130: 186752428,
    131: 209297771, 132: 233962072, 133: 260939787, 134: 290442814, 135: 322702028, 136: 357968938, 137: 396517495, 138: 438646053, 139: 484679494, 140: 534971538,
    141: 589907252, 142: 649905763, 143: 715423218, 144: 786955977, 145: 865044093, 146: 950275074, 147: 1043287971, 148: 1144777804, 149: 1255500373, 150: 1376277458,
    151: 1508002470, 152: 1651646566, 153: 1808265285, 154: 1979005730, 155: 2165114358, 156: 2367945418, 157: 2588970089, 158: 2829786381, 159: 3092129857, 160: 3377885250,
    161: 3689099031, 162: 4027993033, 163: 4396979184, 164: 4798675471, 165: 5235923207, 166: 5711805728, 167: 6229668624, 168: 6793141628, 169: 7406162301, 170: 8073001662,
    171: 8798291902, 172: 9587056372, 173: 10444742007, 174: 11377254401, 175: 12390995728,
    176: 13492905745, 177: 14690506120, 178: 15991948361, 179: 17406065609, 180: 18942428633,
    181: 20611406335, 182: 22424231139, 183: 24393069640, 184: 26531098945, 185: 28852589138, 186: 31372992363, 187: 34109039054, 188: 37078841860, 189: 40302007875, 190: 43799759843,
    191: 47595067021, 192: 51712786465, 193: 56179815564, 194: 61025256696, 195: 66280594953, 196: 71979889960, 197: 78159982881, 198: 84860719814, 199: 92125192822, 200: 100000000000
};
...
function renderMerchantWarehouse() {
    ...
    elements.merchantWarehouseGrid.querySelectorAll('.merchant-warehouse-item').forEach(itemEl => {
        itemEl.addEventListener('click', function(e) {
            ...
            const count = getItemCountFromObject({ type, id });
...
function getItemCountFromObject(item) {
...
function getItemSellPrice(item) {
    // 代币
    if (item.type === 'token' || item.type === 'tokens') {
        return ITEM_VALUES.tokens[item.id] || 50;
    }
    // 木材
    if (item.type === 'wood') {
        return ITEM_VALUES.woods[item.id] || 2;
    }
    // 矿石
    if (item.type === 'ore') {
        return ITEM_VALUES.ores[item.id] || 2;
    }
    // 采集品
    if (item.type === 'gathering') {
        return ITEM_VALUES.gathering[item.id] || 2;
    }
    // 木板
    if (item.type === 'plank') {
        return ITEM_VALUES.planks[item.id] || 16;
    }
    // 矿锭
    if (item.type === 'ingot') {
        return ITEM_VALUES.ingots[item.id] || 16;
    }
    // 布料
    if (item.type === 'fabric') {
        return ITEM_VALUES.fabrics[item.id] || 16;
    }
    // 精华
    if (item.type === 'essence') {
        return ITEM_VALUES.essences[item.id] || 16;
    }
    // 药水
    if (item.type === 'potion') {
        return ITEM_VALUES.potions[item.id] || 28;
    }
    // 酒类
    if (item.type === 'brew') {
        return ITEM_VALUES.brews[item.id] || 118;
    }
    // 工具
    if (item.type === 'tool') {
        // 先检查锤子
        if (ITEM_VALUES.hammers[item.id]) {
            return ITEM_VALUES.hammers[item.id];
        }
        // 其他工具
        return ITEM_VALUES.tools_normal[item.id] || 116;
    }
    return 1;
}
...
function executePendingAction() {
    ...
    // 使用新的统一行动系统
    if (type === 'woodcutting') {
        startGenericAction('WOODCUTTING', id, count);
    } else if (type === 'mining') {
        startGenericAction('MINING', id, count);
    } else if (type === 'gathering_item' || type === 'gathering_all') {
        startGenericAction('GATHERING', id, count, { locationId: id, itemId: itemId });
    } else if (type === 'crafting') {
        startGenericAction('CRAFTING', id, count);
    } else if (type === 'forging') {
        startGenericAction('FORGING', id, count);
    } else if (type === 'forging_tool') {
        // 锻造工具需要特殊处理（暂保留旧函数）
        startForgingToolWithCount(id, count, itemId.toolType, itemId.toolIndex);
    } else if (type === 'tailoring') {
        startGenericAction('TAILORING', id, count);
    } else if (type === 'alchemy') {
        startGenericAction('ALCHEMY', id, count);
    } else if (type === 'essence') {
        startGenericAction('ESSENCE', id, count);
    } else if (type === 'brew') {
        startGenericAction('BREWING', id, count);
    }
    
    pendingAction = null;
}

function startWoodcuttingWithCount(id, count) {
    startGenericAction('WOODCUTTING', id, count);
}

function startMiningWithCount(id, count) {
    startGenericAction('MINING', id, count);
}
...
function startGatheringWithCount(type, locationId, itemId, count) {
    startGenericAction('GATHERING', locationId, count, { locationId, itemId });
}
...
function updateActionStatusBar() {
    ...
    if (gameState.currentAction) {
        ...
        // 次数显示已由 setActionState 统一处理，这里不再重复添加
...
function startCraftingWithCount(id, count) {
    startGenericAction('CRAFTING', id, count);
}
...
function startForgingWithCount(id, count) {
    startGenericAction('FORGING', id, count);
}
...
function startTailoringWithCount(id, count) {
    startGenericAction('TAILORING', id, count);
}
...
function startBrewingWithCount(id, count) {
    startGenericAction('BREWING', id, count);
}
...
function startEssenceExtraction(essenceId, count) {
    startGenericAction('ESSENCE', essenceId, count);
}

function startAlchemyWithCount(id, count) {
    startGenericAction('ALCHEMY', id, count);
}
